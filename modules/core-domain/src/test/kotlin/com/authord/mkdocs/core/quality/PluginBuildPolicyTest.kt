...
class PluginBuildPolicyTest {
    ...
    fun `build configuration enables intellij platform plugin and runIde readiness`() {
        ...
        val uiPluginBuild = Files.readString(root.resolve("modules/ui-plugin/build.gradle.kts"))
        ...
        assertTrue(uiPluginBuild.contains("intellij {"), "UI plugin module must configure IntelliJ plugin target")
        assertTrue(uiPluginBuild.contains("patchPluginXml"), "UI plugin module must configure plugin.xml compatibility range")
