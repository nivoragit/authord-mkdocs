...
class PluginDescriptorRegistrationTest {
    ...
    fun `plugin descriptor registers platform dependency tool window and start action`() {
        ...
        val pluginXmlPath = root.resolve("src/main/resources/META-INF/plugin.xml")
        ...
        assertTrue(content.contains("serviceImplementation=\"com.authord.mkdocs.ui.intellij.AuthordSplitEditorLayoutStateService\""))
    ...
    fun `markdown toolbar descriptor registers authord preview action in markdown toolbar`() {
        val root = findRepoRoot()
        val markdownDescriptorPath = root.resolve("src/main/resources/META-INF/markdown-toolbar.xml")
